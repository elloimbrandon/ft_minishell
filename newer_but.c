...
/*   Updated: 2020/01/13 02:20:43 by brfeltz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void     ft_print_2d(char **two_d)
{
    ...
    i = -1;
    while(two_d[++i])
        ft_printf("%s ", two_d[i]); /// took away new line
}
...
static int    ft_same(t_env *env, char **input_copy)
{
    int i;
    int len;
    char *temp;
    ...
    i = 0;
    temp = NULL;
    len = ft_strlen(env->exec);
    while(env->exec[len] != '/')
        len--;
    len++;
    temp = ft_strdup(env->exec + len);
...
void    ft_fork(t_env *env, char **input_copy)
{
    char *exec;
    pid_t child_p;

    exec = NULL;
    exec = ft_strdup(env->exec);
    free(env->exec);
    child_p = fork();
...
static void    exec_cmd(t_env *env, struct stat buf, char **input_copy)
{
    ...
    else if (lstat(input_copy[0], &buf) != -1)
    {
        ...
        else if (S_ISREG(buf.st_mode))
        {
            ...
            {
                free(env->exec);
                env->exec = ft_strdup(input_copy[0]);
                ft_fork(env, input_copy);
            }
        }
    }
...
static int     double_check(char *path, t_env *env, char **input_copy)
{
    struct stat buf;

    if (lstat(env->exec, &buf))
    {
        //printf("hitting\n");
        exec_cmd(env, buf, input_copy);
        return(1);
    }
    else
        return(0);
...
static void     bin_cmd(char **path, t_env *env, char **input_copy)
{
    int i;
    ...
    while(path[++i])
    {
        if (ft_same(env, input_copy) == 0)
            env->exec = ft_strdup(input_copy[0]);
        else
            env->exec = build_path(input_copy[0],path[i]);
        if(double_check(path[i], env, input_copy) == 1)
            ft_strdel(path);
        else
        {
            printf("error\n");
...
static void    ft_local_exec(char **input_copy, t_cmd *input_check, t_env *env)
{
    if (ft_strchr(input_copy[0], '/') != NULL && ft_strchr(input_copy[0], '.') != NULL)
    {
        if (access(input_copy[0], F_OK) == 0)
        {
            ...
            {
                env->exec = ft_strdup(input_copy[0] + 2); // dupes after "./"
                ft_fork(env, input_copy);
            }
            else
